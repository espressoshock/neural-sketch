#!/usr/bin/env lua
-- Usage: lua generate_variants.lua [--double|-D] <texfile> [target_directory] [output_basename]
-- This script requires pdflatex, pdf2svg, and mv (or an equivalent copy command on your system).

-- Parse optional flags.
local double_compile = false
local clean_args = {}
for _, v in ipairs(arg) do
	if v == "--double" or v == "-D" then
		double_compile = true
	else
...
if #arg < 1 then
	print("Usage: lua " .. arg[0] .. " [--double|-D] <texfile> [target_directory] [output_basename]")
	os.exit(1)
end
...
compile_job(light_job, light_tex)
